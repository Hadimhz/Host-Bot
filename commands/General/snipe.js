...
module.exports.run = async (client, message, args) => {
    if (args[0] != null) {
        ...
        if (args[0].toLowerCase() == 'purge' && (message.member.roles.cache.get(config.discord.roles.staff) != null || message.member.roles.cache.get(config.discord.roles.botdev) != null)) {
            ...
            client.channels.cache.get('868596254567989248').send({content: `${message.member} purged ${target == '*'? `all messages`: `${target}'s messages in ${message.channel}`} for the reason: ${reason}.`, attachment: [file]})
